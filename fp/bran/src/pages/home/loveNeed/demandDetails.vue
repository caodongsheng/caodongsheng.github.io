<template>
  <div class="needDetail-container">
    <div v-if="vos">
      ...
      <swiper :options="swiperOption" ref="mySwiper" class="scrollswiper">
        <swiper-slide v-for="(item1,index1) in vos.imgUrl" :key="index1">
          <div
            ...
            @click="show(index1)"
            :style="{backgroundImage:'url('+getmidpic(item1)+')',backgroundSize:'cover',backgroundPosition:'center'}"
          ></div>
        </swiper-slide>
      </swiper>
      ...
      <div v-transfer-dom>
        <previewer v-if="ispreviewer" :list="demandImgList" ref="previewer" :options="options" :style="getTop" @on-close="close"></previewer>
      </div>
...
export default {
  ...
  methods: {
    ...
    close(){
      window.android.listenNativeGoBack(0)
    },
    end(){
    ...
    show(index1) {
      ...
      window.android.listenNativeGoBack(1)
    }
  },
  mounted() {
    setTimeout(() => {
