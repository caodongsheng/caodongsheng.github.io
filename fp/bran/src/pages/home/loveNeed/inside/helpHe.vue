  ...
  <div class="payWrapper">
    ...
    <div class="box-withheader over-auto">
      ...
      <div class="submitContainer">
        <!-- <check-icon :value.sync="agreeX"> 我已阅读并同意（用户协议）</check-icon> -->
        <div class="tip">
...
export default {
  ...
  data() {
    return {
      ...
      prepayId: "", // 预支付id
      isN:false,
      anonymousId:process.env.anonymousUserid

    };
  },
  created(){
    if(this.$route.query.isN == 1){
      this.isN = false
    }else{
      this.isN = true
      this.anonymousId = ''
       ...
       // this.payType = true;
        ...
        this.$http
          ...
          .then(response => {
            ...
            let payData = {};
            let userzcId=_this.G("userid")?_this.G("userid"):this.$route.query.zcuserid
            if(this.$route.query.isN == 1){
              payData.userId = this.anonymousId
            }else{
              payData.userId = userzcId;
            }
         ...
         //   payData.spbillCreateIp = "127.0.0.1";
            ...
            _this.$http
              ...
              .then(function(response) {
                // _this.prepayId = response.prepayid;
                // _this.prepayId = response.data.prepayid;
                localStorage.setItem('isFirstPay',2)
                console.log('document.referrer',document.referrer,'localStorage.payUrlLink',localStorage.payUrlLink)
                var fromUrl=''
                if(_this.frompath){
                  fromUrl=_this.frompath
                ...
                let linku = process.env.PJ_URL+'payview/certificate.html?ordernum='+_this.orderNumber+"&jdomain=" +encodeURIComponent(process.env.BASE_API)+'&money='+_this.submitMonet+'&token='+localStorage.getItem("token")+'&userid='+userzcId+'&demandcode='+_this.$route.params.id+'&payIsN='+!_this.isN+"&path="+encodeURIComponent(fromUrl)
                let linkStr  = response.data.mweb_url+ "&redirect_url=" + encodeURIComponent(linku) 
  ...
  watch: {
    ...
    inputMoney: function(val) {
      if (val !== "") {
            this.$nextTick(() => {
            this.inputMoney = val.replace(/\D/g,'');
