<template>
  <div class="needProve-container">
    ...
    <div class="content-container">
      ...
      <div class="complaint" v-if="user_type">
        <!-- <router-link :to="{name:'report',params:{id:this.paramsID}}"> 质疑／举报</router-link> -->
    ...
    <p class="reference-title">
      已有
      <span class="c-a xiahua" @click="showProveList" v-if="user_type">{{peopleCount}}</span>
      <span class="c-a xiahua" v-else>{{peopleCount}}</span>
      人为他证明
      <!-- <router-link :to="{name:'iToProve',params:{id:this.paramsID}}">
        <span class="toproveLink">我来证明</span>
      </router-link>-->
      <router-link to @click.native="toProve" v-if="user_type">
        <span class="toproveLink">我来证明</span>
      </router-link>
    </p>
    <ul v-if="peopleCount>0" :class="{'reference-list':true,'active':provePeople}">
      ...
      <div v-if="provePeople && user_type" class="moreProve">
  ...
  methods: {
   ...
   async toReport() {
       if (!this.G("token")) {
        ...
        {
          this.$router.push({ name: "report", params: { id: this.paramsID } });
        }
      }
    },
      onConfirmPhone() {
    ...
    async toProve() {
      ...
      await this.$store.dispatch('getJhUser', { _this: this.$http })
    ...
    showProveList() {
      this.$router.push("/proveList/" + this.paramsID);
    },
    onConfirm() {
      // this.$router.push({
      //   path: "/login",
      ...
      this.ModuleNative('AppReturnToLoginPage','证明去登录')
    },
    toRealName() {
