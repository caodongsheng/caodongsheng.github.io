    ...
    <div class="box-withheader over-auto">
      <transition name="bounce">
        <div v-show="imgBig" class="imgModal" @click="modalCloseOpen">
          <img v-lazy="getmidpic(imgBigSrc)" style="margin: 0 auto;width: 100%;display: inline-block">
        </div>
      </transition>
      ...
      <div class="flex">
        <div>需求详情</div>
        <div class="tpl" @click="useTpl">详细描述示例</div>
      </div>
      <div class="textarea">
        <x-textarea
          ...
          v-model="description"
          :class="getClass"
          placeholder="建议详细描述您的需求：如家庭背景，收入情况，需求原因等，请输入至少150个字的内容"
          @on-blur="changevx"
        ></x-textarea>
      </div>
...
<!--       <div>
      ...
      <div>
        <div class="item">
          <div style="display: flex;flex-wrap: wrap;padding:1em .6em">
            <div v-for="(value,index) in imgArr" :key="'asdasdsd'+index" style="width: 33%;margin-bottom:5px;text-align: center;position: relative">
              <div
                @click="imgToBig(value)"
                style="width: 90%;height: 7em;"
                :style="{backgroundImage:'url('+value+')',backgroundSize:'cover',backgroundPosition:'center'}"
...
export default {
 ...
 destroyed() {
  ...
  computed: {
    ...
    getClass() {
      if (this.description.replace(/(^\s*)|(\s*$)/g, "").length < 150) {
        return "redfont";
      } else {
        return "";
      }
  ...
  methods: {
    clickUpload(){
      if(!this.isios()){
        let len= 9
        window.android.getPicSize(len-this.imgArr.length)
      }
    },
    uploadImg(e){
      let that=this;
      ...
      if(e.target.files.length+that.imgArr.length==9){
      ...
      }else if(e.target.files.length+that.imgArr.length>=10){
        this.showToast("", "text", "最多只能上传9张图片，请重新上传", "20em", "middle");
      }else{
        for( var i = 0 ; i < imgFiles.length ; i++ ){
          var fr = new FileReader();
          fr.onload = function(e){
    ...
    moneyInputBlur() {
      ...
      if (this.money > this.end_money && this.end_money) {
        this.showToast("", "text", "金额不可超过" + this.end_money);
    ...
    async publish() {
      ...
      if (this.isCapitalDemand) {
        ...
        // }
        if (this.money > 20000) {
          this.showToast("", "text", "金额不可超过20,000元");
          this.money = 20000;
          return;
        }
        if (this.money <= 0) {
      ...
      let obj = checkDesc(this.description.replace(/(^\s*)|(\s*$)/g, ""),150,5000,"需求详情")
      if(!obj.valid){
