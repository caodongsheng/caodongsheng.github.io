      ...
      <div>
        <div class="item">
          <div style="display: flex;flex-wrap: wrap;padding:1em .6em">
            <div
              v-for="(value,index) in imgArr" :key="index+'asdsdasd'" style="width: 33%;margin-bottom:5px;text-align: center;position: relative">
              <div
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    next(vm => {
      //console.log('进来2')
      if (from.path.includes("aidedAddressList") && vm.$route.meta.addressIndex !== -1) {
        //console.log('进来3')
        vm.getAddress();
      }
      if (from.path.includes("aidedaddress") &&vm.$route.meta.addressIndex !== -1) {
        //console.log('进来3')
        vm.getAddress();
      }
    });
  ...
  methods: {
    ...
    async publish() {
      ...
      // if (this.descfail) {
        ...
        // else{
        //   if (!/[^\x00-\xff]{150,5000}$/.test(this.description.replace(/(^\s*)|(\s*$)/g, "")))
        //   {
