                ...
                <div class="dis-block linkonly" @click="getHelpType(item.demand_type,item.id)">
                  <div class="flex-tab">
                    <div class="flex-tab-cell tab-left">
                      <img
                        v-lazy="getmidpic(item.img_url) ||'../../static/appimg/rec/slh3.png'"
                        width="80"
                        height="80"
                      >
                    </div>
              ...
              <div class="list-state clearfix">
                ...
                <div class="display-flex right-btn-contatiner">
                  <a
                    ...
                    @click="goasd(btn.url,item.id,demandtype,item,btn)"
...
<script>
import Vue from 'vue'
import { XHeader, XImg, Spinner,AlertPlugin } from "vux";
...
export default {
  ...
  methods: {
    goasd(url,demandId,code,item,btn){
      // :to="{name:btn.url,params:{demandId:item.id,code:demandtype}}"
      console.log(url,demandId,code,'=====',item.demand_status,'--',item.isSpecial,'-',btn.text)
      if(item.demand_status==4 && btn.text.indexOf('提现')>-1 && item.isSpecial==1){
        this.$vux.alert.show({
          title: '提示',
        ...
        this.$router.push({ name: url, params: { demandId: demandId, code:code} });
      }
