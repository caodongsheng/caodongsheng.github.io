<template>
  ...
  <div class="pub-state bg-ffffff  f-14">
    <span class="color-999">需求编号：</span><span v-clipboard:copy="demandDetail.id" v-clipboard:success="copy" v-clipboard:error="onError" class="copyneedId">{{demandDetail.id}}</span>
    ...
    <div class="state-timeline-content">
      <timeline>
        <timeline-item v-for="(item,index) in timeline" :key="index">
          ...
          <!-- <h5 class="recent f-normal f-14" v-if="demandDetail.demand_status==2">{{item.staff_name+item.op_result}}</h5> -->
          <h6 v-if='item.remarks' class="recent f-normal f-14" style="word-wrap:break-word;word-break:break-all;"><span v-if="item.op_result.indexOf('通过') !== -1 || item.op_result.indexOf('终审') !== -1">{{item.staff_name}}</span>&nbsp;<span v-if="item.op_result.indexOf('不通过') !== -1">{{item.op_result}}<br/>不通过理由：</span>{{item.remarks}} <span v-if="item.op_result.indexOf('发布') !== -1">{{item.op_result}}</span></h6>
...
<script>
  import {XHeader, Group, Cell, CellBox, XButton, Timeline, TimelineItem, Range, TransferDom} from 'vux'
      import STATUS_MAP_UI from '../STATUS_MAP_UI'
  export default {
    name: "timelineList",
    components: {XHeader, Group, Cell, CellBox, XButton, Timeline, TimelineItem, Range, TransferDom},
    props: {
      demandDetail: ''
    },
    created() {
    ...
    methods: {
      getTimeline(demandId) {
        this.$http.post('hts/management/logs', {demandId: this.demandDetail.id})
          ...
          this.showToast('', 'text', '获取需求状态失败')
        })
      },
      copy(e) {
        this.showToast("", "text", "复制成功", "", "middle");
       },
      onError(e) {
        this.showToast("", "text", "复制失败", "", "middle");
      }
    }
  }
</script>
...
<style scoped>
  ...
  .copyneedId{
    padding-left: 20px;
    background: url("~@/assets/images/copy.png") no-repeat left center;
