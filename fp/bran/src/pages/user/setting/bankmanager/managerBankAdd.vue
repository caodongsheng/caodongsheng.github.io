  ...
  <div>
    ...
    <div class="box-withheader over-auto bg-f6f5f6">
      <group class="group-clearbd mt-12 bg-ffffff" label-width="7.5em">
        <x-input title="银行名称" placeholder="请输入银行名称" @on-blur="inputFocusout" @on-focus="inputFocus" v-model.trim="bank_name" :required="true" ></x-input>
        <x-input title="开户行名称" placeholder="请输入开户行名称" @on-blur="inputFocusout" @on-focus="inputFocus" v-model.trim="open_bank"  :required="true" ></x-input>
        <x-input title="收款人" placeholder="请输入收款人姓名" @on-blur="inputFocusout" @on-focus="inputFocus" v-model.trim="payee" :is-type="checkName" :required="true" :min="2" :max="15"></x-input>
        <x-input title="银行账号" placeholder="请输入银行卡号" @on-blur="inputFocusout" @on-focus="inputFocus" v-model.trim="bank_account"  keyboard="number" :required="true" :is-type="checkAccount"  ></x-input>
      </group>
