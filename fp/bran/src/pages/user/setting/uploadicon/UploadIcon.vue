<template>
  <div>
    ...
    -->
    <group>
      <template v-if="!isavata">
          <cell title="头像" is-link @click.native="setShowMenu">
        <!-- <input type="file" accept="image/jpg,image/png,image/jpeg,image/gif"  capture="camera" @change="change" style="display: none;" ref="changetype"> -->
        ...
        <input
          ...
          ref="changetype"
          id="fileinput"
          style="height:35px;position: absolute; right:0px;opacity: 0;top:-18px;-ms-filter: 'alpha(opacity=0)'"
        >
      </cell>
      </template>
    <template v-else>
...
export default {
  ...
  watch:{
    getmask(val){
      if(val){
        this.$nextTick(() =>{
        Bus.$emit("closefooter", true);
        })
 
      }else{
        this.$nextTick(() =>{
            Bus.$emit("closefooter", false);
        })
      }
    }
  },
      ...
      this.$http
        ...
        .then(function(response) {
          if (response.data === "SUCCESS") {
            ...
            self.destoried();
            self.$store.commit("CHANGE_AVATAR", imgurl);
          } else {
            // alert(response.msg)
