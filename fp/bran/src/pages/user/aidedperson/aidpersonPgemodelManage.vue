<template>
  <div>
    <x-header :left-options="{backText: '',preventGoBack: true}" @on-click-back="backToPre">
      受助人管理
    </x-header>
            ...
            <div class="managers">
              <ul>
                <li v-for="(item2,index) in aidedList" :key="index" @click="_chooseOne(item2)">
                  <div class="leaderCon">
                    <avatar width="40" height="40" :image="imgCom(item2.user_pic)" :name="item2.aided_person_name"></avatar>
...
<script>
  ...
  export default {
    ...
    methods: {
      imgCom(imgValue){
        if(imgValue&&(imgValue !="null" || imgValue !=null)){
          if(imgValue.indexOf("http") != -1){
            return imgValue
          }else{
            return process.env.userImg+imgValue
          }
        }else{
          return imgValue
        }
      ...
      getAideds(done) {
        ...
        let self = this;
        this.$http.post('hts/poor/allSearch', {
    ...
    created() {
      this.regionId = this.$store.state.city.id
      this.getAideds();
    },
    mounted() {
