<template lang="html">
  <div class="box">
    ...
    <div class="cart">
      <div class="cent" v-for="item in list" :key="item.pic">
        <router-link @click.native="getLink(item.link)" to="">
          <img v-lazy="getmidpic(item.pic)" alt="">
          <p>{{item.name}}</p>
        </router-link>
      </div>
    ...
    <div class="Region" @click="toregionlist" style="margin-bottom:15px;">
      <div class="clearfix">
    ...
    <div class="Region" @click="gohunan" v-if="isshow">
      <div class="clearfix">
        <img src="../../assets/images/icon_6.png" alt="">
        <span>户帮户亲帮亲</span>
        <span class="num" >{{hunannum}}</span>
      </div>
    </div>
 ...
 import { getMidPic } from "@/util/getMid";
  ...
  export default {
    ...
    created() {
      ...
      let _this = this
      this.$http.get('tas/hunan/getTotalByAdminId',{params:{adminUserid:this.G("userid")}}).then(function(data){
        console.log(data,'222222222222');
        _this.hunannum = data.data
      })

      this.$store.dispatch("getUser",{_this:this.$http}).then(data =>{
      ...
      //   console.log(this.$route.query.id,'ididididididididid111')
      // console.log(this.data,'levelNamelevelNamelevelNamelevelName2222')

      console.log(JSON.parse(this.G("city")).id)
      console.log(this.data.levelName)
      if(JSON.parse(this.G("city")).id == '430000000000' && this.data.levelName == '村级管理员'){
        this.isshow = true;
      }
    ...
    methods: {
      ...
      gohunan(){
        window.location.href=process.env.DJRES_URL_DEV+"appia630/pages/offlineHelpList/offlineAuditListNew.html?backParam=1&userId="+this.G("userid")+"&token="+this.G("token")

        // 4239897038&token=9135db10be34542c2fa09b247b59d5d8834495eb810690662e9cdce8bc38838ac5982928739eb76e56e7b7d70375dce62e833e9967b65b84a15b4ce833c3c5142a1f773d97110011&isApp=1username=15810000003&userid=4239897038"
      },
       getmidpic(imgurl){
    ...
    computed: {
      usericon: function () {
        return this.userdata.user_pic?this.userdata.user_pic:""
      },
      vtype: function () {
        if (this.userdata.is_cadre&&this.userdata.is_cadre === 1) {
          return '1'
        } else return '0'
      },
      isCertification: function () {
        return this.userdata.is_celebrity&&this.userdata.is_celebrity === 1
      },
      username: function () {
        return this.userdata.nick_name?this.userdata.nick_name:""
      },
      authunion: function () {
        if (this.userdata.org_name&&this.userdata.org_name !== '') {
          return '认证机构：' + this.userdata.org_name
        } else return ''
      }
    }
  }
