<template>
  <div v-if="demandDetail.demand_status!==undefined">
    ...
    <div class="box-withheader over-auto bg-f6f5f6">
      ...
      <!--收货地址-->
      <div class="address bg-ffffff mb-12" v-if="demandDetail.demand_type == 1">
      ...
      <div class="helprecord bg-ffffff ">
        <group class="group-clearbd">
          <cell title="帮扶记录" is-link :link="{name:'mypubgoodsrecord',params:{demandId:demandId}}">
            <span class="f-14">共<em class="f-14 color-orange"> {{demandDetail.record_count}}</em>次</span>
          </cell>
          <div v-if="demandDetail.record_count==0" class="noRecord text-center color-999">暂无最新捐赠记录</div>
        </group>
        <div  v-if="demandDetail.record_count>0" class="recordshow">
          ...
          <swiper :options="swiperOption" ref="mySwiper" class="scrollswiper">
            <swiper-slide v-for="(item1,index1) in goodsImglist" :key="index1">
              <img v-lazy="getmidpic(tem1.src)" class="previewer-demo-img" @click="show(index1)">
            </swiper-slide>
          </swiper>
          <div v-transfer-dom>
      ...
      <!--v-if=面对面，捐赠人信息-->
      <div class="donor bg-ffffff mb-12" v-if="demandDetail.help_pattern===1 &&demandDetail.donor_link_man !== null">
      ...
      <!--v-if=快递，物流信息-->
      <div class="transport bg-ffffff mb-12" v-if="demandDetail.help_pattern===2 && demandDetail.express_no !== null">
        <group class="group-clearbd">
          <cell title="物流信息" ></cell>
        </group>
        <group class="group-clearbd" v-if="demandDetail.express_no !== null">
          <cell-box  class="trans-con">
            <div class="color-999 f-14">
              <p>快递公司：{{demandDetail.express_org}}</p>
              <p>物流单号：{{demandDetail.express_no}}</p>
            </div>
          </cell-box>
        </group>
      </div>
...
<script>
 ...
 import { getMidPic } from "@/util/getMid";
  ...
  export default {
    ...
    components: {XHeader, Group, Cell, CellBox, XButton, Range, Confirm,swiper, swiperSlide, Previewer,headerInfo,basicInfo,loveNews,timelineList,buttonActions},
    name: "mypub_capital",
     created() {
