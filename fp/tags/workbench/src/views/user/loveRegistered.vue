  ...
  methods: {
      resetPass(id){
          let data={
            ...
            axios.get(this.BASE_URL+"/ssoredis/login/l002/resetPassword", {
                params: data})
              .then(res => {
                  this.$notify({
                    title: "提示",
      ...
      if (this.selectarr.length > 0) {
        ...
        this.exporthandler(
          process.env.BASE_API +
            `ets/excel/firstConcernExcelDown?token=${this.user.token}&id=${idstr}`,
          obj
        );
      } else {
        ...
        urlFunction(urlobj).then(res => {
          ...
          this.exporthandler(
            process.env.BASE_API +
              `ets/excel/firstConcernExcelDown?token=${this.user.token}${datastr}`,
            obj
          );
        });
      }
    ...
    async exporthandler(url, obj) {
      ...
      let exporttotal = 0;
      await this.$http
        .post("/ets/excel/firstConcernExcelDownCount", obj)
        .then(response => {
    ...
    async getList() {
      ...
      await this.$http
        .post("/uts/user/firstConcern", this.listQuery)
        .then(res => {
          if (res.list.length) {
            this.list = res.list;
            setTimeout(() => {
