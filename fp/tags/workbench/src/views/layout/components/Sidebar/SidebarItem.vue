      ...
      <template v-for="child in item.children" v-if="!child.hidden">
        ...
        <router-link v-else :to="resolvePath(child.path)" :key="child.name">
          <el-menu-item :index="resolvePath(child.path)">
            <item v-if="child.meta || (Number(this.$store.state.user.leaderLevel) !== 5 && child.meta.title === '扶贫干部审核')" :icon="child.meta.icon" :title="child.meta.title" />
          </el-menu-item>
        </router-link>
      </template>
    </el-submenu>
    ...
    methods: {
      hasOneShowingChild(children) {
        ...
        if (showingChildren.length === 1) {
          if(showingChildren[0].meta.title=="扶贫干部查询" || showingChildren[0].meta.title=="青鹰职位管理" || showingChildren[0].meta.title=="户帮户亲帮亲查询")
          {
            return false
          }
          return true
