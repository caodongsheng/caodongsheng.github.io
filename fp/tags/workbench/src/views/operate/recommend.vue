<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-form>
        <el-row>
          ...
          <el-button size="mini" type="primary" icon="el-icon-search" style="margin-top:3px;" @click="getList">查询</el-button>
    ...
    <el-table v-loading="listLoading" :key="tableKey" :data="list"  border highlight-current-row stripe>
      ...
      <el-table-column align="center" label="区划" prop="allParentRegionName"/>
      <el-table-column align="center" label="帮扶类型"  :formatter="helpTypeFn"/>
      <el-table-column align="center" label="封面图片">
          <template slot-scope="scope">
              <img :src="scope.row.bannerPicUrl" alt="加载失败" class="coverPic">
          </template>
      </el-table-column>
      <el-table-column align="center" label="链接地址" prop="bannerLinkUrl"/>
      <el-table-column align="center" label="操作" min-width="160px" >
        <template slot-scope="scope">
          <el-button size="mini" type="primary" style="background:#23bab7;border-color:#23bab7;" @click="updtaBenner(scope.row)">修改</el-button>
          <el-button size="mini" type="primary"  style="background:#23bab7;border-color:#23bab7;" @click="updataRecom(scope.row)">推荐</el-button>
          <el-button size="mini" type="primary"  style="background:#23bab7;border-color:#23bab7;" @click="goRecomList(scope.row)">查看推荐需求</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <add-recommend :visible.sync='visible'  ref="addBanner" :bannerid="bannerid" @upData="getList"></add-recommend>
    <recom-detail :visible.sync='visible2'  ref="recomDetail" :bannerid="bannerid" :type="type" @upData="getList"></recom-detail>
  </div>
</template>
...
import addRecommend from '@/views/operate/addRecommend'
...
export default {
  ...
  components:{linkage,addRecommend,recomDetail},
  data() {
    return {
      visible:false,
      visible2:false,
  ...
  methods: {
      goRecomList(row){
          this.$router.push('/operates/recomList?type='+row.id+'&regionId='+row.regionId)
      },
      helpTypeFn(row){
         let str
      ...
      updataRecom(row){
          this.visible2=true
          this.$refs.recomDetail.getData('',row.helpTypeCode,row.regionId,row.id);
      },
      newsBanenr(){
          this.bannerid=''
          this.visible=true
      ...
      this.$http.post('hts/management/getColumnList', this.listQuery).then(response => {
