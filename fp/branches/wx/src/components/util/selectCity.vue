      ...
      <popup v-model="show">
        <div>
          ...
          <div class="bg-ffffff" v-show="showselect">
            <div style="line-height:34px;padding-top:5px;padding-left:15px">已选 ↓</div>
            <div  class="bg-f6f5f6" style="line-height: 24px;padding: 10px 15px">
              <span v-show="provinceobj">{{provinceobj?provinceobj.regionName+" —":''}}</span>
              <span v-show="cityobj">{{cityobj?cityobj.regionName+" —":''}}</span>
              <span v-show="districtobj">{{districtobj?districtobj.regionName+" —":''}}</span>
              <span v-show="townobj"> {{townobj?townobj.regionName+" —":''}}</span>
          ...
          <div style="background-color: white;border-bottom:#e7e7e7 1px solid" class="areadiv">
            <div><span @click="provinceSelected()" :class="showProvince?'active':''">省份</span></div>
            <div><span @click="citySelected()" :class="showCity?'active':''">城市</span></div>
            <div><span @click="districtSelected()" :class="showDistrict?'active':''">区/县</span></div>
            <div><span @click="townSelected()" :class="showTown?'active':''">乡/镇</span></div>
            <div><span @click="villageSelected()" :class="showVillage?'active':''">村</span></div>
          </div>

        </div>
        <ul class="ulclss" style="height:350px;overflow:scroll;background:white">
          <li class="addList" v-for="(v,k) in info" :key="(v,k).index"
              @click="getProvinceId(v.id, v.regionName,k)"
              v-show="showProvince"
              :class="v.selected === true ? 'active' : ''">{{v.regionName}}<span v-if="v.selected === true"
                                                                                 class="spanchecked"></span></li>
          <li class="addList" v-for="(v,k) in showCityList" :key="(v,k).index"
              @click="getCityId(v.id, v.regionName,k)"
              v-show="showCity"
              :class="v.selected ? 'active' : ''">{{v.regionName}}<span v-if="v.selected === true"
                                                                        class="spanchecked"></span></li>
          <li class="addList" v-for="(v,k) in showDistrictList" :key="(v,k).index"
              @click="getDistrictId(v.id, v.regionName,k)"
              v-show="showDistrict"
              :class="v.selected ? 'active' : ''">{{v.regionName}}<span v-if="v.selected === true"
                                                                        class="spanchecked"></span></li>
          <li class="addList" v-for="(v,k) in showTownList" :key="(v,k).index"
              @click="getTownId(v.id, v.regionName,k)"
              v-show="showTown"
              :class="v.selected ? 'active' : ''">{{v.regionName}}<span v-if="v.selected === true"
                                                                        class="spanchecked"></span></li>
          <li class="addList" v-for="(v,k) in showVillageList" :key="(v,k).index"
              @click="getVillageId(v.id, v.regionName,k)"
              v-show="showVillage"
              :class="v.selected ? 'active' : ''">{{v.regionName}}<span v-if="v.selected === true"
                                                                        class="spanchecked"></span></li>
        </ul>
      ...
      getValue(val) {
        ...
        if (val.length > 4 || val.length > 3) {
          ...
          self.getCityList(val[0].id, function () {
                // console.log('aaaa' + self.provinceobj.regionName);
          });
