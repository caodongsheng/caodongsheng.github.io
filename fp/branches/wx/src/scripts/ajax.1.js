...
// 添加请求拦截器
let yltoken = localStorage.getItem('token')
...
instance.interceptors.request.use((config) => {
  ...
  if (md.os()=="iOS"||md.os()=="AndroidOS") {
    config.headers.clientType = '1'// app
  } else {
  ...
  switch (code) {
    case 'I0000':
      ...
      if (!tokenLock) {
        tokenLock = true
        //调取原生方法
        let newtoken = window.getUserInfo().accToken;
        console.log(yltoken,newtoken)
        localStorage.setItem("token",newtoken)
       if(localStorage.getItem('token')==null||localStorage.getItem('token')==yltoken){
        // tokenLock = true;
