...
<script>
  ...
  export default {
    ...
    methods:{
       getRate() {
        if (this.vos.moneyCount) {
          let num = this.vos.moneyCount / this.vos.targetMount * 100
       
          let fixNum = new Number(num).toFixed(1);//四舍五入之前加1  
          return parseInt(fixNum*10)/10
        }else if (this.vos.moneycount && this.vos.moneycount !== 0) {
              let num = this.vos.moneycount / this.vos.target_amount * 100
         // let fixNum = new Number(num).toFixed(1);//四舍五入之前加1  
