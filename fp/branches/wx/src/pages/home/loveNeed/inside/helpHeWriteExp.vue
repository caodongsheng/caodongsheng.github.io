<template>
  <div>
    ...
    <x-header :left-options="{backText: '',preventGoBack: true}" @on-click-back="backToPre">{{type == 2? '填写快递':'填写信息'}}</x-header>
    <div class="box-withheader over-auto bg-f6f5f6">
      ...
      <div class="address bg-ffffff mb-12">
        ...
        <div class="clearfix name-area f-14">
          <span class="f_l"><em>收货人：</em> {{aidedObj.aideder_name}}</span><span class="f_r">{{aidedObj.aideder_tel}}</span>
        </div>
      ...
      <group class="group-clearbd bg-ffffff" label-width="4em" label-margin-right="2em" label-align="left">
        <popup-picker class="labelStyle" v-if="type==2" title="快递公司" :data="companyList" v-model="selectCompany" value-text-align="left"></popup-picker>
        <x-input class="f-16 labelStyle" :is-type="companyOrdercheck" :required="true" v-for="(item,index) in expressList" :key="index" :title="item.title" :placeholder="item.default"  v-model="item.model" v-if="type==2"></x-input>
        <x-input class="f-16 labelStyle" type="text" v-for="(item,index) in ftfList" :required="true" :key="index" :title="item.title" :placeholder="item.default"  v-model="item.model" v-if="type==1" :is-type="checkNameG" :min="2" :max="15"></x-input>
        <x-input class="f-16 labelStyle" title="联系人" :required="true" placeholder="请填写您的姓名" v-model="donorName"   :is-type="checkName" :min="2" :max="15"></x-input>
        <x-input class="f-16 labelStyle"  :is-type="checkphonefun" :required="true"  title="手机号码" placeholder="请填写11位手机号码"  v-model="relPhone"   keyboard="number" :max="11"></x-input>
...
<script>
  import {XHeader, Group, Cell, CellBox, XButton, XInput,TransferDom,PopupPicker,InlineLoading } from 'vux'
  import {mapState} from 'vuex';
  import {checkPhoneStr,checkNameStr} from '@/util/validFun.js';
  export default {
    components: {XHeader, Group, Cell, CellBox, XButton, XInput,PopupPicker,InlineLoading},
    name: "helpHeWriteExp",
    directives: {
      TransferDom
    },
    data() {
      return {
       ...
       // companyList:[['圆通速递','申通速递','百世快递','中通快递','顺丰速递','宅急送','全峰快递','德邦物流','优速物流','京东快递','信丰快递','跨越速运','邮政平邮','天天快递','快捷速运','亚马逊','韵达快递']],
        ...
        typeList:[],
        aidedObj:{},
        user: this.$store.state.user.realname ? this.$store.state.user.realname : this.$store.state.user.nick_name,
        demanduse: '',
        aidedname: '',
        donorName: '',
        relPhone: '',
        ...
        routePath: '',
        phonefail:true,
        expressList:[{title:'快递单号',default:'请填写快递单号',model:''}],
    ...
    ...mapState(['allexpress']),
    companyList(){
      return [this.allexpress.logistics_company.map(x => {return x.name})]
    }
  },
    created(){
    ...
    methods: {
      companyOrdercheck(value){
       ...
       let reg = new RegExp(arr[0].reg_mail_no)
        if(reg.test(value)){ 
        ...
        backToPre() {
         ...
         checkName(value) {
          // if(!/^[\u4E00-\u9FA5\uf900-\ufa2d·s]{2,15}$/.test(value)) {
          //  this.namefail = true
          //     return {valid:false,msg:"姓名输入有误"}            
          // }
          //   else {
          //      this.namefail = false
          //     return {valid:true}    
      ...
      faceSubmit(){
        ...
        let _this=this;
        this.$http.post('hts/record/addFace',{
          ...
          .then(function(response){
            ...
            _this.$router.replace({path: '/certificate', query: {user: _this.user, to: _this.aidedname, target: _this.demanduse}})
          })
      ...
      expressSubmit(){
           ...
           this.showPrompt = true
        let _this=this;
        this.$http.post('hts/record/addExpress',{
          'id':this.$route.params.id,
        //   'demand_code':
     ...
     checkphonefun(value){
      ...
      footSubmit(){
        ...
        //   }
          ...
          //快递配送
          if(this.expressfail) {
              this.showToast('','text',this.selectCompany[0]+'单号输入有误','','middle')
              return
          }
...
.vux-x-input>>> .weui-label {
  ...
  .weui-cell>>> input{color:#999;font-size: 12px}
  .labelStyle{color: #333;font-size: 14px}
  /*content*/
  ..box-withheader{bottom: 44px;}
  /*底部按钮*/
  .btn-opearea{position: absolute;bottom:0;left:0;right:0;border-radius:0!important;height:44px!important;color: #fff;font-size: 15px;text-align:center;background:linear-gradient(-180deg, #fd8b2a, #ff8125);line-height:44px!important;box-shadow:none;}
  /*收货地址部分*/
  .address .name-area {
    padding: 15px 15px;
  ...
  .address .address-title{color: #333;font-size: 15px}
  .address .add-area {
