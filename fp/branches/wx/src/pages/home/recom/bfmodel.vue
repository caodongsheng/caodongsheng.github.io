<template lang="html">
  <div class="bg-f6f5f6">
    <div class="mt-15" v-for="(item,index) in helpdata" :key="index" @click="goitem(index+1)">
      <div class="panel-box">
        <cell :title="item.help_type_name" value="更多" :link="item.link_url" is-link>

          <img slot="icon" width="22" style="display:block;margin-right:5px;"
               :src="item.icon_url"/>
        </cell>
        ...
        <div class="scrollswiper-wrap">
          <swiper :options="swiperOption" ref="mySwiper" class="scrollswiper">
            ...
            <swiper-slide v-for="(option,index) in item.recommendList" :key="index">
              ...
              <router-link to="" @click.native="todetail({name:'loveNeed',params:{id:option.demand_id}},item.help_type_name)" class="boxlist-item">
                ...
                <img :src="option.cover_pic" class="listitem-img"><!--轮播图-->
                <h1 class="f-12 f-bold tit-nowrap" v-if="item.help_type_code!=='4'">{{option.demand_title?option.demand_title:option.re_desc}}</h1>
                ...
                <p class="clearfix" v-if="item.help_type_code!=='4'">
