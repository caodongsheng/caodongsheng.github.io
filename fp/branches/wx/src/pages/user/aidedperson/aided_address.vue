...
<script>
  import {XHeader, Group, Cell, XInput, XTextarea, CheckIcon, XSwitch,InlineLoading} from 'vux'
  import MyAddress from '@/components/util/myaddressnew3'
  import {mapState} from 'vuex';
  import {checkAddress,checkPostcardStr} from '@/util/validFun.js';
  export default {
    name: "aided_address",
    components: {XHeader, Group, Cell, XInput, XTextarea, MyAddress, CheckIcon, XSwitch,InlineLoading},
    data() {
      return {
    ...
    methods: {
     ...
     let validobj = checkAddress(this.detailAddr, 4, 50, "详细地址");
      ...
      saveaddressinfo() {
        ...
        if (this.type == "add") {
          this.$http.post('hts/poor/addressSave', obj)
            .then(function (response) {
                ...
                if (self.routeFrom.includes('aidedAddressList')) {
                  self.$store.dispatch('getAidederAddressList', arg).then(() => {
                      self.showLoading=false
                    self.showPrompt = false
                    self.$router.go(-1)
                  }).catch(err => {
          ...
          obj.id = this.id
          this.$http.post('hts/poor/addressEdit', obj)
            .then(function (response) {
                //console.log("修改成功")
