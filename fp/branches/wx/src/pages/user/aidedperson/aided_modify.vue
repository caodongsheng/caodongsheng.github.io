<template>
  <div>
    <x-header :left-options="{backText: ''}">信息修改<a slot="right" class="color-orange f-16" @click="saveuserinfo()">保存</a></x-header>
    <div class="box-withheader over-auto bg-f6f5f6">
      <group class="group-clearbd mt-12" label-width="10em">
        ...
        <x-input title="受助人身份证号" placeholder="" v-model="id_card"  text-align="right" disabled class="border-bottom-1px"></x-input>
        <!-- <cell title="受助人所在地">
           <span class="color-999">湖南省永州市永州县永州乡镇永州村</span>
         </cell>-->
        <my-address class="ipt" title="所在地" :readonly="true" placeholder="受助人所在地"
                    @confirmAction='confirmFun'
                    @hidePicker='hidePicker'
                    v-model="addressvalue" ref="address" ></my-address>
        <x-input title="受助人联系方式" v-model="userPhone"  placeholder="请输入11位手机号" text-align="right" :is-type="checkphonefun" :required="true" keyboard="number" :max="11"></x-input>
        <popup-picker title="您与受助者关系" :data="relationlist"  v-model="relation" show-name :columns="1" :required=true :disabled="relationReadOnly" @on-hide="confirmValue" ></popup-picker>
      </group>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods:{
      ...
      saveuserinfo(){
        ...
        this.$http.post('hts/poor/userEdit', {
        	helpId: this.helpId,
      ...
      confirmValue() {
        //   this.relation = 
            ...
            if(this.relation[0] < 10) {
                this.relation[0] = this.relationlist[this.relation[0].substr(1,1)-1].value
            }else {
    ...
    created(){
      ...
      console.log(userinfo.relation_id)
       ...
       let arr= this.alljson.recipients.find(item =>
           item.value === userinfo.relation_id|| item.name === userinfo.relation_id
       )
