  ...
  <scroller
    ...
    <slot>
        <div class="helplist-wrap bg-f6f5f6">
            <capitalitem :item="item" v-for="(item,index) in helplist" :key="index"></capitalitem>
        </div>   
    </slot>
   ...
   <div  v-else class="nocontent f-14 color-999 text-center bg-f6f5f6" style="padding: 12px 15px">
        ...
        <p style="padding-top: 10px">抱歉，您还没有资金帮扶记录</p>
      </div>
</template>
...
import { setTimeout } from 'timers';
  ...
  export default {
    ...
    name: "record_capital",
    components:{ Badge,capitalitem,scrolltemplate},
    data(){
      return{
        // end:false,
        capitalpage: 1,
    ...
    getCapitalList(done) {
        this.$http.post('hts/record/capitals',{page: this.page})
        .then(res => {
