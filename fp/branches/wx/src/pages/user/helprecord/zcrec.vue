  ...
  <scroller
    ...
    <slot>
        <div class="helplist-wrap bg-f6f5f6" style="padding-top:15px;">
            <zcitem :item="item" v-for="(item,index) in helplist" :key="index"></zcitem>
        </div>
    </slot>
   ...
   <div  v-else class="nocontent f-14 color-999 text-center bg-f6f5f6" style="padding: 12px 15px">
        <img class="dis-block" src="../../../../static/appimg/no-content.png" style="width:90px;margin:0 auto">
        <p style="padding-top: 10px">抱歉，您还没有众筹帮扶记录</p>
      </div>
</template>
...
import { setTimeout } from 'timers';
  ...
  export default {
    ...
    getCapitalList(done) {
        // this.$http.post('cfs/zc/project/getHelpRecordList',{page: this.page},)
        // .then(res => {
        ...
        if(this.userId){
            this.$https({
                method: 'post',
                url: this.BASE_URL+'cfs/zc/project/getHelpRecordList',
                data: {
                    pageIndex:this.page ,
                    pageSize: 5,
                    acr026:this.userId,
