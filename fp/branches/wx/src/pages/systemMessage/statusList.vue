         ...
         <div class="status-item-message mt-12">
          <div class="status-item-message-img" v-if="item.img&&item.img.length>0">
            ...
            <swiper-slide v-for="(item1,index1) in item.img" :key="index1">
              <div class="previewer-demo-img" style="padding-top:100%" @click="show(index1,index)"  v-lazy:background-image="item1" :style="{backgroundSize:'cover',backgroundPosition:'center'}">
              </div>
            </swiper-slide>
          </swiper>
           <div v-transfer-dom>
  ...
  methods: {
    ...
    helpSpread (item) {
      let voss = {};
       ...
       voss.link = this.WinXinHost + '#/loveNeed/' + item.demand_code
       voss.imgUrl = item.img&&item.img.length>0?item.img[0]:"../../../static/appimg/share_icon.png"
